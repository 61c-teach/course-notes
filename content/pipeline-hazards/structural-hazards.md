...
By contrast, if we used a single memory block that could only support one read/write at a time (to any part of memory—instruction or data), then `inst5` and `inst2` attempting to access memory in the same cycle would cause a structural hazard.
