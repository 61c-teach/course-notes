...
* **Complex instructions**: CPI >> 1, if used in a program benchmark. For example, take an ISA that specifies the C `strcpy` function as an assembly-level `strpcy`. The assembly-level `strcpy` will necessarily incur more cycles to than, say, an assembly-level `add`.
